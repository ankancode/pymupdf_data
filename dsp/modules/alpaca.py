...
class Alpaca:
    def __init__(self, model='text-davinci-002', api_key=None, **kwargs):
        ...
        self.kwargs = {'model': model, 'temperature': 0.0, 'max_tokens': 150, 'top_p': 1,
                       'frequency_penalty': 0, 'presence_penalty': 0, 'n': 1, **kwargs} # TODO: add kwargs above for </s>
    ...
    def basic_request(self, prompt, **kwargs):
        ...
        response = alpaca_api(kwargs, url="")
        # response = "E. L. Doctorow"
        # from openai.openai_object import OpenAIObject
        response = convert_to_openai_format(response)
...
import requests
def alpaca_api(kwargs, url):
    response = requests.post(url, data=kwargs)
    return response.json()


def convert_to_openai_format(response):
    openai_format_response = {
        ...
        "created": 1681898901,
        "id": "cmpl-76z1ZOCct8gts2klwtorcGXUvrYgj",
